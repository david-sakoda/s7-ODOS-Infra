...
data:
  mapRoles: |
    - rolearn: ${aws_iam_role.preprod-node.arn}
...
clusters:
- cluster:
    server: ${aws_eks_cluster.preprod-cluster.endpoint}
    certificate-authority-data: ${aws_eks_cluster.preprod-cluster.certificate_authority[0].data}
  name: kubernetes
contexts:
- context:
    cluster: kubernetes
...
output "jenkins_ip_address" {
  value = "${aws_instance.web_facing_instance.public_dns}"
}
